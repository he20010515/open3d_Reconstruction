...
class NewOrbit(Orbit):
    ...
    def __init__(self, AXIS  # 小行星旋转的轴 enum("X","Y","Z")
                 , starName  # 小行星名字
                 , FRAME_PRE_SECOND, deltaTheta, deltaPhi, start_time: float, end_time: float, R: float):
        super().__init__(FRAME_PRE_SECOND, deltaTheta, deltaPhi, start_time, end_time, R)
        self.FRAME_PRE_SECOND = FRAME_PRE_SECOND
        self.deltaTheta = deltaTheta  # 维度变换速度
        self.deltaPhi = deltaPhi  # 经度变换速度
        self.start_time = start_time
    ...
    def orbit_scan(self):
        ...
        while start <= end:
            ...
            bpy.data.objects['Camera'].location[1] = y
            bpy.data.objects['Camera'].location[2] = z
            assert isinstance(self.AXIS, str)

            u = start / self.FRAME_PRE_SECOND * self.deltaPhi
            if self.AXIS.lower() == "x":
                bpy.data.objects[self.starName].rotation_euler = (u, 0, 0)
            elif self.AXIS.lower() == "y":
                bpy.data.objects[self.starName].rotation_euler = (0, u, 0)
            else:
                bpy.data.objects[self.starName].rotation_euler = (0, 0, u)
            start += 1
            if self._scan(start):
                pass
            else:
    ...
    def _argument_function(self, nowtime):
        x = self.R * cos(nowtime * self.deltaTheta)
        y = self.R * sin(nowtime * self.deltaTheta)
        z = 0
        return x, y, z
...
if __name__ == '__main__':
    lam = 0.2
    orbit = NewOrbit("X", "Ero", 200, np.pi, 12 * np.pi, 0 + lam, 1 - lam, 20)
